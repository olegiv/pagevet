...
func Handler(logf func(format string, args ...any)) http.Handler {
	...
	mux.HandleFunc("/login-spa", f.loginSPA)
	mux.HandleFunc("/login-secondbutton", f.loginSecondButton)
	mux.HandleFunc("/login-outside", f.loginOutside)
	mux.HandleFunc("/login-evilaction", f.loginEvilAction)
	mux.HandleFunc("/login-tabpass", f.loginTabPassword)
	mux.HandleFunc("/login-outsidesubmit", f.loginOutsideSubmit)
	mux.HandleFunc("/login-lateaction", f.loginLateAction)
	mux.HandleFunc("/login-redirectingpost", f.loginRedirectingPost)
